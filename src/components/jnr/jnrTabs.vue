...
export default {
  ...
  mounted() {
    ...
    if (this.currentId != -1) {
      //默认选中的项
      this.tabsCurrentId = this.currentId;
      this.modifyTab(this.currentId);
    } else {
      console.log("空1111");
    }
  ...
  methods: {
    ...
    modifyTab(tabsCurrentId) {
      ...
      //切换 索引
      this.tabsCurrentId = tabsCurrentId;
      //同时修改 父组件的 id
      this.$emit("tab-modify", tabsCurrentId);
      //代码的形式跳转路由
      //切换内容
      ...
      $(`#jnr-tabs-title-item-label-${tabsCurrentId}`).click();
    },
    // 提供鼠标点击标签事件/切换
    tabClick(target) {
      //显示关闭图标
    ...
    removeTab(id) {
      ...
      this.tabsDataArray = tabs.filter((tab) => {
        // 关闭 index项
        return tab.id != id || !tab.applyClose; //返回关不了的新数组
      });
    },
    ...
    closeRight() {
      ...
      for (let i = this.tabsDataArray.length - 1; i > 0; i--) {
        ...
        this.removeTab(this.tabsDataArray[i].id);
      }
